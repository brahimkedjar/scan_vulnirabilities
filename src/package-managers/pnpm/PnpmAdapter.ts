...
import { parsePnpmDependencies } from "./PnpmDependencyParser";

export class PnpmAdapter implements PackageManagerAdapter {
  public readonly id = "pnpm";
  public readonly displayName = "pnpm";
    ...
    try {
      ...
      for (const project of projectsSelectedForScan(detection, options)) {
        ...
        try {
          ...
          const parsed = parsePnpmDependencies({
          ...
          errors.push({
            ...
            message:
              error instanceof RangeError
                ? error.message
                : "Could not read pnpm dependency metadata",
